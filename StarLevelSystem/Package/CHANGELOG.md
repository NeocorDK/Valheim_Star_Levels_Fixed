...
- CORRECTION: MaxLevel and MaxBossLevel are a STAR count, not a level - GetMaxCreatureLevel resolves them as stars + 1. An earlier entry in this release said the opposite and the description was changed to match; both are put back. MaxLevel 20 means 20 stars
- The level generator page works in stars throughout, which is what the rest of the game shows. Its range is no longer two sliders: it always starts at 0 stars, because a chance table that does not list unstarred creatures cannot say how many stay unstarred, and it ends at the star cap from the Stats page. The two sliders also used a different unit from the cap they sat next to - setting the end to 4 produced a table covering 0 to 3 stars
- The Table editor sets the SHARE of creatures with each number of stars, in percent, rather than raw thresholds. Setting 0 stars to 0 now means "no unstarred creatures" and the rest take its place; as a threshold that same intent had to be written as 100, and typing 0 produced the exact opposite. The stored file format is unchanged
- BEHAVIOUR: the Linear and Exponential curve styles produced distributions that were not what their names say, and both are corrected. Linear ramped the THRESHOLD evenly, and a level's real chance is the gap between two thresholds - so evenly spaced thresholds meant evenly sized gaps and "Linear" was actually uniform: a span of 5 at chance 0.74 gave 26% / 18.5% / 18.5% / 18.5% / 18.5%. Exponential pinned its decay so the threshold hit zero exactly at MaxLevel, which made a HIGHER chance decay FASTER: 0.74 over five levels gave 26 / 66 / 7 / 0.8 / 0.09, so the slider did not push creatures up the range, it moved the pile from the first level to the second. Every style now states its curve as a share per level and is converted to thresholds in one place. LevelUpChance keeps its meaning - the share of creatures above MinLevel - so a chance of 0.25 gives the 75 / 18.8 / 4.7 / 1.2 / 0.3 you would expect. Check any generator you have configured
- The Table curve style has an editor. Selecting it used to leave nothing to edit - the shapes live in LevelupWeightTablesBySpan and the panel had no field for them - so the visible sliders quietly stopped affecting anything. There is now a row per level, seeded from the curve that would otherwise be used, with the graph updating as you type
