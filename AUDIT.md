...
- ✅ исправлено — 48 из 50: U1–U14, L1–L9, L11–L13, C1–C10, Y2–Y13
- 🟡 сознательно ограничено — 2: L10 (поведение оставлено, описание приведено к коду), Y1 (слияние там, где оно обещано; общего дозаполнения намеренно нет)

**Собрать проект в этой среде не удалось:** .NET SDK не установлен, а `StarLevelSystem.csproj` — SDK-style. Правки выверены чтением кода, но компиляцией не проверены.
...
**Статус:** ✅ исправлено (0558e86) — все 134 видимые строки панели переведены в токены `$sls_cfg_*` и заведены в `English.json`: заголовки страниц, подписи ползунков и переключателей, кнопки навигации, строка статуса, 28 описаний модификаторов, слова вокруг чисел в превью масштабирования и кривой. Отсутствующий перевод в другом языковом файле откатывается на английский, а не показывает сырой токен
...
**Статус:** ✅ исправлено (0558e86, дополнено позже) — `ConfigUI.CreatePanel` уменьшает панель, которая не помещается, измеряя родительский rect канваса (он уже в тех же единицах, что и панель, поэтому один расчёт покрывает все сочетания разрешения и `GuiScale`). Отдельно подтвердилось, что предупреждение аудита о хрупкости жёстких смещений было не теоретическим: правая колонка страницы 2 переполнилась ровно так, как описано, — генератор уровней вынесен на собственную страницу и получил график кривой. Остальные страницы помещаются с запасом
...
**Статус:** 🟡 исправлено с одной ошибкой (4c99ad4, 7842603; ошибка исправлена позже) — описания переписаны под фактическое поведение, **кроме строки про `MaxLevel` / `MaxBossLevel`: сама находка аудита оказалась неверной.** Она опиралась на `UpdateLevelsOnChange.cs:51`, а это была рассинхронизированная строка из пункта L9. Авторитетный код — `GetMaxCreatureLevel`, который возвращает `MaxLevel.Value + 1`; ZDO-уровень равен «звёзды + 1», значит **`MaxLevel` — это счётчик звёзд**, и исходное описание «The Maximum number of stars» было верным. Правка по C2 внесла регрессию в описание и в подпись панели; обе возвращены
...
**Статус:** ✅ исправлено (0558e86) — `EnableCustomRaidsCompat` и `EnableJewelCraftingBossHudCompat` переехали в `ModCompat` к двум своим соседям через `MigratedValue` (появилась перегрузка для смены секции). `UseCustomHealthFont` биндится общим помощником, как соседи. Имена секций (`Client config` и прочие) оставлены как есть: переименование стоило бы миграции в файле каждого пользователя ради косметики
...
**Статус:** ✅ исправлено (656861f, уточнено позже) — на подключённом клиенте `.cfg` перечитывается, но `cfg.Reload()` намеренно **не** вызывается: он присваивает значения всем записям подряд, и каждое присваивание синхронно поднимает `SettingChanged`, то есть клиент успел бы прогнать свой `MaxLevel` через `ModifyLoadedCreatureLevels` до того, как серверное значение вернётся на место — а этот проход перезаписывает уровни в ZDO. Вместо этого файл разбирается вручную и в `ConfigEntry` попадают только записи без `IsAdminOnly`; серверные не трогаются вообще, поэтому окна не существует
