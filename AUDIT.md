...
**Статус:** ✅ исправлено (0558e86) — `ConfigUI.CreatePanel` уменьшает панель, которая не помещается, измеряя родительский rect канваса (он уже в тех же единицах, что и панель, поэтому один расчёт покрывает все сочетания разрешения и `GuiScale`). Жёсткие смещения внутри страниц оставлены: сегодня содержимое помещается, а переход на компоновку по потоку — переписывание панели, а не исправление дефекта
